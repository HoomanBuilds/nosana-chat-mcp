...
- Refactored `DeploymentHandler` to support dual auth modes: Wallet (SDK) and API Key (REST).
- Created `apikey.tools.ts` for direct REST API interaction (deployments, credits) without wallet dependencies.
- Updated `deployer.tools.ts` to be strictly wallet-only.
- Dynamic system prompts now prevent AI from asking for wallet addresses in API key mode.

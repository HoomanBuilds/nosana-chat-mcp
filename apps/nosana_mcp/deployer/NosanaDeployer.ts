...
export class NosanaDeployer {
  ...
  private getApiKey(auth?: AuthContext): string {
    ...
    throw new Error('No Nosana API key available. Provide one via auth or set NOSANA_API_KEY env.');
  }
  ...
  private async apiRequest<T = any>(
    ...
    if (!res.ok) {
      ...
      throw new Error(`Nosana API error (${res.status} ${res.statusText}): ${text}`);
    }
  ...
  private async getCreditBalanceViaApi(auth?: AuthContext): Promise<CreditBalance> {
    const data = await this.apiRequest<CreditBalance>('/credits/balance', { auth });
    return {
  ...
  async getMarketViaApi(marketAddress: string, auth?: AuthContext): Promise<any> {
  ...
  private async createJobViaApi(
    ...
    const deploymentPayload: DeploymentCreateParams = {
    ...
    const deployment = await this.apiRequest<DeploymentResponse>(
      '/deployments',
      { method: 'POST', body: deploymentPayload, auth }
    );
    ...
    try {
      ...
      console.warn('Auto-start failed (deployment may need manual start):', startErr);
    }
  ...
  // ── Get Deployment (API key mode) ────────────────────────────────────────

  async getDeployment(deploymentId: string, auth: AuthContext): Promise<any> {
    return this.apiRequest<any>(`/deployments/${deploymentId}`, { auth });
  }

  // ── List Deployments (API key mode) ──────────────────────────────────────

  async listDeployments(auth: AuthContext): Promise<any[]> {
    return this.apiRequest<any[]>('/deployments', { auth });
  }

  // ── Stop Job (dual mode) ─────────────────────────────────────────────────

  async stopJob(jobId: string, auth?: AuthContext): Promise<{ ok: boolean; tx?: string; note?: string }> {
    // API key mode
    if (auth?.mode === 'api_key') {
    ...
    if (apiKey) {
      ...
      const response = await fetch(`${this.getApiBase()}/jobs/stop-with-credits`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${apiKey}`,
          'Content-Type': 'application/json',
  ...
  private async stopJobViaApi(
    ...
    try {
      const result = await this.apiRequest<any>(
        `/jobs/stop`,
        { method: 'POST', body: { address: jobId }, auth }
      );
      return {
        ok: true,
      ...
      try {
        const result = await this.apiRequest<any>(
          ...
          { method: 'POST', body: { jobAddress: jobId }, auth }
        );
        return {
          ok: true,
  ...
  async stopDeployment(
    ...
    return { ok: true, note: 'Deployment stopped via Nosana API' };
  }

  // ── Archive Deployment (API key mode) ────────────────────────────────────
  ...
  async archiveDeployment(
    ...
    return { ok: true, note: 'Deployment archived via Nosana API' };
  }
  ...
  private async extendJobViaApi(
    ...
    const result = await this.apiRequest<any>(
      ...
      {
        method: 'POST',
        body: { address: jobId, timeout: Math.ceil(extensionSeconds / 60) },
        auth,
      }
    );
    return {

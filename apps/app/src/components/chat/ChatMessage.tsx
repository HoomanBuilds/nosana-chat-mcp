...
const ChatMessage = memo(({ msg, index, conversations, setQuery, textareaRef }: any) => {
    const { tool } = useChatStore();
    const { localConfig: { appearance } } = useSettingsStore();
    ...
    return (
        <div className={cn("flex mt-8 justify-start mb-4")}>
            <div className="min-w-[95%] w-[100%] flex flex-col gap-2">
                ...
                {/* search Result */}
                {msg.search && msg.search.length > 0 && <SearchResultsSection search={msg.search} />}

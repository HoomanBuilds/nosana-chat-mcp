...
async function extendJobViaApiKey(
    ...
    if (!res.ok) {
        ...
        if (!res2.ok) {
            ...
            throw new Error(`Failed to extend job via API: ${res2.status} ${errText}`);
        }

        const data = await res2.json();
        return {
            txSig: data.transactionId ?? '',
            result: { result: `Job extended by ${extraMinutes} minutes via API (credits).` },
        };
    }
    ...
    return {
        txSig: data.transactionId ?? '',
        result: { result: `Job extended by ${extraMinutes} minutes via Nosana API.` },
    };
}

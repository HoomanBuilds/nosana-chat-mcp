...
import { chatJSON, checkHuggingFaceModel, checkJobExtendable, checkJobStop, createJobDefination, fail } from "./utils/helpers";
import { getResolverPrompt, suggest_model_market_prompt } from "./prompt/deployer.prompt";
import { DecisionSchema, TResult, suggest_model_market_schema } from "./utils/schema";
...
export const createJob = tool({
  ...
  inputSchema: z.object({
    ...
    market: z.enum(DEFAULT_MARKETS).optional().describe("find compatible market for model, market should be capable enough to run this model, you can use related tool first to get suggestions and based on them run tool"),
    ...
    timeoutSeconds: z.number().min(600).max(86400 * 7).default(3600).describe("Default is 1 hour if the user does not provide many details. Just start the tool with timing and other details."),
    userPublicKey: z.string(),
    directJobDef: z.record(z.string(), z.any()).optional().describe("ONLY use this for COMPLETE Nosana job definitions that include ALL of: 'type', 'ops' array, and 'meta' object. Do NOT use for partial JSONs like {id, type, source} - instead extract info to 'model' and 'requirements'."),
  }),
...
Please adjust your configuration or specify a compatible market manually.`);
        ...
        try {
          ...
          console.warn(`⚠️ Could not fetch Job cost for market: ${market_public_key}, timeoutSeconds: ${params.timeoutSeconds}`, err);
        }

        return {
          tool_execute: true,
          args: { ...params, marketPubKey: market_public_key, provider: isHF ? "huggingface" : "container" },
...
`,
    ...
    if (!params.model || !params.requirements) {
      return fail("Either provide 'directJobDef' (complete JSON) OR 'model' + 'requirements' for AI generation.");
    }
    ...
    try {
      ...
      if ((err as Error).message.includes("overloaded")) return fail("❌ chatJSON failed: Ai model Overloaded. take a break before request get refresh");
      return fail(
        `❌ chatJSON failed: ${(err as Error).message}. Try calling createJob again or notify the user.`
      );
    }

    if (!result) return fail("❌ No resolver result found. Ask user to retry job creation.");

    function getGpuMarket(requireVram: number) {
      const match = Object.values(MARKETS)
        .find(m => m.vram_gb >= requireVram);
...
- "Find a smaller variant of ${params.model}"
    ...
    if (Model_vram == null) return fail(`market selection issue : Model_vram undefined , select a existing market , run get market , and choose the market satify the VRAM ${result.vRAM_required} GB requirement and try creating Job againd`);
    if (Model_vram < result.vRAM_required) return fail(`❌ Model ${params.model} requires more vram than provided by market ${params.market}
    ...
    if (result.providerName === "huggingface") {
      ...
      if (modelInfo.status !== 200) return fail(`Model not reachable (does not exist) (${modelInfo.status}) . try some other similar model you have in your mind or run getModels_from_tags tool to get some models`);
      if (modelInfo.private)
        return fail(`⚠️ ${result.modelName} is private on Hugging Face. try some other similar model you have in your mind or run suggest_model_market tool to get some models`);
      if (modelInfo.gated)
        return fail(`🔒 ${result.modelName} is gated on Hugging Face. try some other similar model you have in your mind or run suggest_model_market tool to get some models`);
      ...
      try {
        ...
        return fail(`Job definition validation crashed:\n${(err as Error).message}`);
      }

      if (!validation?.success) {
        console.error("❌ Validation failed:", JSON.stringify(validation.errors));
        return fail(
      ...
      try {
        ...
        if (!finalCheck.success) {
          console.error("❌ Final validation before return failed:", JSON.stringify(finalCheck.errors));
          return fail(
        ...
        return fail(`Job definition validation crashed before return: ${(err as Error).message}, try resolving again`);
      }
    } else {
      return fail(`⚠️ Unsupported provider: ${result.providerName}, createJob again with better details dont end stream, only container | huggingface supported`);
    }
...
export const stopJob = tool({
  ...
  execute: async ({ jobId, job_owners_pubKey }) => {
    ...
    if (!jobResult) return fail(`❌ Job ${jobId} not found. JOB SHOULD BE EXISTING TO BE STOPPED`)
    const { job } = jobResult

    console.log(job);
    if (!job) return fail(`❌ Job ${jobId} not found. JOB SHOULD BE EXISTING TO BE STOPPED`)
...
export const extendJobRuntime = tool({
  ...
  inputSchema: z.object({
    jobId: z.string(),
    extensionSeconds: z.number().max(86400).min(60).describe("Maximum extension time is 24 hours."),
  ...
  execute: async ({ jobId, extensionSeconds, job_owners_pubKey }) => {
    ...
    const validation = checkJobExtendable(job, job_owners_pubKey, extensionSeconds);
    if (validation) return validation;
...
export const getJob = tool({
  ...
  execute: async ({ jobId }) => {
    try {
      ...
      const marketEntry = Object.entries(MARKETS).find(([, m]) => m.address === job.market.toString());
...
export const getAllJobs = tool({
  ...
  inputSchema: z.object({
    ...
    state: z.enum(["QUEUED", "RUNNING", "COMPLETED", "STOPPED", "ALL"]).optional().default("ALL").describe("don't ask user until user from his side dont mention keep all as default"),
  }),

  execute: async ({ userPubKey, state }) => {
    try {
      const deployer = ensureDeployer();
      const jobs = await deployer.getAllJobs(userPubKey, { limit: 50, state: state ?? "ALL" });
...
export const getWalletBalance = tool({
  description: "Fetches Solana + NOS token balances for the connected Nosana account. additinally give you : current solana and nosana usd price",
...
export const getMarket = tool({
  ...
  inputSchema: z.object({
    gpuMarket_slug: z.enum(DEFAULT_MARKETS).optional().describe("either slug or address one is must both are not optional"),
    gpuMarket_address: z.string().optional().describe("either address or slug one is must both are not optional"),
  }),
  execute: async ({ gpuMarket_slug, gpuMarket_address }) => {
    try {
      const deployer = ensureDeployer();
      const { market, nos, gpu_usd, marketName } = await deployer.get_market(gpuMarket_slug as GpuMarketSlug, gpuMarket_address as string);
...
export const listGpuMarkets = tool({
  description: "Lists all supported Nosana GPU markets and their metadata. give price | vram | slug | address",
  inputSchema: z.object({}),
  execute: async () => {
    try {
...
export const suggest_model_market = tool({
  description: "Suggests the best GPU markets and models based on user requirements.",
  inputSchema: z.object({
...
export const getModels_from_tags = tool({
  description: "give hugging face model list from tags, you just use it for for him, used when same name models are giving error",
  inputSchema: z.object({
    modelProvider: z.string().describe("organization name ex deepseek-ai , mistralai , qwen , qwen2 , qwen1.5 , google etc."),
    ...
    `),
    pipeline: z.enum(["image-text-to-text", "text-generation"]).describe("the pipeline name")
  }),
  execute: async ({ tags, modelProvider, pipeline }) => {
    const deployer = ensureDeployer()
    const models = await deployer.getModels({ pipeline: pipeline, keywords: tags, organization: modelProvider })
...
export const validate_job_definition = tool({
  description: "Validates and debugs a Nosana job definition JSON. Use this when user provides a complete JSON config or wants to check their config for errors. Does NOT execute the job, only validates syntax and structure.",
  inputSchema: z.object({
    jobDefinition: z.record(z.string(), z.any()).describe("The complete job definition JSON object to validate"),
    strict: z.boolean().optional().default(true).describe("If true, performs strict validation. If false, only checks critical fields."),
  }),
...
`,
      ...
      if (jobDefinition.ops?.[0]?.args?.env?.MODEL_ID) {
        ...
        if (!jobDefinition.ops[0].args.env?.HF_TOKEN && !jobDefinition.ops[0].args.env?.HUGGING_FACE_HUB_TOKEN) {
          warnings.push(`Model ${modelId} might be gated or private. Consider adding HF_TOKEN if needed.`);
        }
      }
